...
const { createUser, getUser, addTokenToBlacklist, sendCoupleRequest, acceptCoupleRequest, checkCoupleStatus } = require('../service/user.service')
...
class UserController {
    ...
    async register(ctx, next) {
        ...
        try {
            ...
            const res = await createUser(name, yier_number, password)
    ...
    async login(ctx, next) {
        ...
        try {
            ...
            const { password, id, ...res } = await getUser({ yier_number })
    ...
    async getUserInfo(ctx, next) {
        ...
        try {
            ...
            // 调用服务层方法获取用户信息
            const { password, ...userInfo } = await getUser({ id: userId });
    ...
    async linkCouple(ctx, next) {
        ...
        try {
            ...
            // 假设在验证令牌的中间件中已经将用户 ID 存储在 ctx.state.userID
            const userId = ctx.state.userID;
            ...
            const isCoupled = await checkCoupleStatus(userId, targetUserId);
            if (isCoupled) {
                ctx.body = {
                    code: 400, // 状态码 400 表示客户端请求错误
            ...
            if (action === 'send') {
                ...
                await sendCoupleRequest(userId, targetUserId);
                ctx.body = {
                    code: 0, // 状态码 0 表示操作成功
                ...
                await acceptCoupleRequest(userId, targetUserId);
                ctx.body = {
                    code: 0, // 状态码 0 表示操作成功
